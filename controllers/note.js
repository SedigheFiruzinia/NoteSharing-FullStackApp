...
noteRouter.get('/:id', async (request, response) => {
    ...
    const notesByOwner = populated.filter(note => note.owner._id.toString() === request.params.id)
...
noteRouter.post('/', async (request, response, next) => {
    ...
    try {
        ...
        await userSchema.findByIdAndUpdate(owner._id, {notes:owner.notes.concat(savedNote._id)} , { new: true })
...
noteRouter.post('/:id',async(request,response,next) => {
    ...
    try{
        ...
        await noteSchema.findByIdAndUpdate (note._id, {sharedWith:note.sharedWith.concat(userToShare)}, { new: true })
        const user = await userSchema.findById(userToShare)
        await userSchema.findByIdAndUpdate(user._id, {sharedNotes:user.sharedNotes.concat(note._id)})
